...
\end{equation}
The number of features used is then increased until the target rates are met for this stage. The number of stages to be trained is a hyperparameter. Training further stages is usually stopped early if $acceptanceRatio$ (negative samples wrongly classified as positive) is reached, which prevents over-fitting of training data. According to \citep{vjdataset}, $acceptanceRatioBreakValue=10^{-5}$ has empirically been found to be a good value. Smaller values may indicate an over-fit.

In line with \cite{robovj} the following parameters are set: $minHitRate = 0.999$. $maxFalseAlarmRate=0.5$. $numPos$ is chosen to be 90\% the positive samples, which is a conservative figure that provides a buffer of images to compensate for the positive cases falsely not detected anymore by the classifier. A common choice for $numNeg$ is twice $numPos$. $NumStages$ is found by increasing the number of stages and evaluating the AP score for convergence. $minNeighbors$ is then fine tuned to maximize the AP. The remaining parameters are recommended values by \cite{vjdataset} or default.  
...
An ablation study is now performed in order to achieve a real time detector. A search is performed to optimize the $scaleFactor$.
