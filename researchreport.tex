...
Initially, the raw detections of the tracking dataset are studied alongside the ground truth ball position as well as the ball position inferred by a perfect detection. In the latter case, the ground truth detections are forward transformed to field ball positions using the same transformation as used by the agent. A ``latch'' is used for tracking, which means that missed and intermediate steps are filled by the previous positions.
...
In figure \ref{fig:rawerror} the root mean square error has been computed over binned kick distance. It can be seen that a ``noise floor'' exists due to the presence of inaccuracies in the forward transformation of the agent. A reason for this is that a simplified representation of the the agent model is used in the team which does not compensate for offsets in angular uprightness of the agent, which have generally been seen as providing an acceptable level of error compared to the complexity of a more accurate representation. Since the FatProxy agent walks on the spot when not commanded, the agent is constantly in motion and error is continuously introduced.

At the near zero distance (before the Fat Proxy kick causes non-physical error) the position error is dominated by the error in the transformation. Up until approximately 4m distance from the agent, a significant portion of error can also be attributed to the transformation model. However, beyond this point, it is evident that the error explodes. 
...
In these kicks the following can be noted. Firstly, the error due to the transformation can be observed in the time domain, where it is clear that the cyclic nature of the agent motions results a significant error, as previously noted. Secondly, the inference of distance beyond .. appears problematic
