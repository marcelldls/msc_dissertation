...
NanoDet-Plus v1.0.0 (d3fb34f) is cloned from \cite{nanodet}, which is the latest release of the repository at this time. The deep-learning frame work, Pytorch, is used, however only versions < 2.0.0 are supported, which must be manually installed before installing the required packages to get a suitable environment.

\subsubsection{Data preparation}

The NanoDet model expects data to be presented in a COCO style format. 

\subsubsection{Model training}

During an initial study, using the pretrained model, it is found that the model achieves a useful performance on the target dataset - even though it is synthetic. This supports the approach of fine-tuning the pretrained model 

In order to prove the hypothesis, it is found that the default parameters for the ... model is sufficient. It is chosen to train each of the default models 

\begin{figure}[h!]
\begin{center}
\includegraphics[width=10cm]{images/nanodet_train.png}
\caption{AP, latency vs $scaleFactor$}
\label{fig:vjspeeds}
\end{center}
\end{figure}

An interesting result is obtained with the pretrained Nanodet model, which is seen to have some level of performance. This supports the decision the apporach used

\begin{figure}[h!]
\begin{center}
\includegraphics[width=8cm]{images/nanodet_compare.png}
\caption{AP, latency vs $scaleFactor$}
\label{fig:vjspeeds}
\end{center}
\end{figure}

As a post training step, the repository tools are used to produce the model in an ONNX format. A crucial step which is not documented by \cite{nanodet} is to post-process the ONNX model using the the ONNX runtime $remove\_initializer\_from\_input$ function. Without this step, a significant (almost half) reduction in inference speed occurs which impact the ability of the detector to run in realtime.

\newpage
\section{Evaluation of detection}

The selected trained and fine-tuned models are now compared relative to each other using the unseen test dataset. The results are deconstructed and evaluated by the relative size of the objects, which helps to provide an indication into the performance of the detector against distant target objects. Models that are pretrained on the Coco dataset are also included as they provide insight for how well the model generalizes to the synthetic dataset.
