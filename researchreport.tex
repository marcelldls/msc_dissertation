...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=14cm]{images/plotdetections.png}
...
The Viola Jones classifier (more generally known as the haar cascade classifier) is implemented in the OpenCV libraries. It can be trained, stored and loaded for performing inference - much like the modern object detectors of today.

\subsubsection{Software environment}

It is found that OpenCV no longer supports the training of Viola Jones style detectors after v3 (although, inference is still supported). Therefore it is chosen to roll back to the vx.x.x release. 

\subsubsection{Model training}
With the detection data prepared accordingly.
