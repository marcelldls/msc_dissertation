...
Generalized Focal Loss (GFL) combines classification and regression to provide one score

$L_{QFL}$ and $L_{DFL}$ are the Quality and Distributed Focal losses respectively. $L_{GIoU}$ is the Generalized IoU loss \citep{giouloss}.

 QFL and DFL can be unified into a general form, which
is called the Generalized Focal Loss (GFL

QFL combines classification and IoU quality to provide one score

Generalized Focal Loss as classification and regression loss.

Where $\lambda$ and $\lambda'$ are balancing weights used to regulate the importance of each term inorder to give importance to bounding box regression and decrease the importance of boxes which do not contain objects (AKA ``noobj'') respectively. $L_{cls} $ refers to the class prediction which is represented by the focal loss. $L_{confi}$ refers to the objectness score (which is the probability that and object is present) which is represented by the $L2$ loss. $L_{reg} $ refers to the bounding box regression loss. YOLOv4 applies a loss known as ``Complete-IOU'' (CIoU) \citep{diouloss} (rather than a basic $L2$ loss) which offers an objective which is made to suit to the IoU evaluation metric. CIoU solves the ``gradient-vanishing`` problem which refers to an issue of a basic IoU loss which would only ``work'' when the bounding boxes overlap. 
