...
In the next phase of the project, the tracking problem is considered by using the detector from phase one for tracking-by-detection. An EKF with a simple RoboCup world model (similar to \cite{kalmanmodel} and  \cite{3dparabola}) is implemented as a baseline tracker. A tracking dataset is generated using the RoboCup simulator and used to train and optimize neural assisted Kalman tracking. The trackers performances are then compared on a test dataset with RMSE as a performance measure.

\subsection{Dataset generation}

To generate a tracking dataset, a series of randomized kicks are performed in order to capture image and ground truth position data that describes various trajectories. Kicks are performed from random locations on the training pitch in order to capture a variety of scenes. The tracking dataset is produced by applying the object detector for inference which can be transformed into coordinates relative to the robot frame of reference. The ground truth ball data is directly read from the server at the default sampling rate of 50hz. This rate is faster than the camera, which means that intermediate estimations (those without correction) are evaluated.
...
The tracking performance of the trackers is evaluated and compared using the RMSE performance measure on the test dataset. The performance target is to process images at a minimum of 25 FPS, which is generally accepted as real-time execution.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
