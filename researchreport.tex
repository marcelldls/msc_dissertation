...
YOLO object detection is based on the DarkNet deep learning framework, which is purpose built for training and evaluating the YOLO architecture in the object detection task. A pretrained YOLO v4 model is made available by the author, which has been trained on the Coco dataset, as well as the configuration and pretrained backbone that have been used to get the reported results. Since the Coco dataset features a ``sports ball'' category, it is possible to apply the pretrained model to the Robocup ball dataset in order to evaluate how well the models generalize between the domains as well as performance gain that is achieved at the cost of additional training.
...
\subsubsection{Software environment}
 ...
 The ``YOLO V4'' release is cloned from the official repository hosted on github \citep{yolov4repo}. CUDA 11.8 is linked to the makefile - in order to benefit from GPU acceleration - and then the framework is compiled. The training dataset needs to be prepared in a ``DarkNet style'' format. The online deep-learning service ``Roboflow'' \citep{roboflow} is utilized as a means of validating the produced annotations by importing and visualizing the data. 
...
It can be noted that the $AP_{@[IoU=0.5]}$ score of the 480 x 480 resolution model achieves a near perfect result. This gives confidence that although the architecture has been prepared for the Coco dataset, it is capable of capturing the patterns within the data.
