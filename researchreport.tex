...
\end{equation}
The number of features used is then increased until the target rates are met for this stage. The number of stages to be trained is a hyperparameter. Training may be stopped early if $acceptanceRatio$ is reached, which prevents over-fitting of training data. According to \citep{vjdataset}, $acceptanceRatioBreakValue=10^{-5}$ has empirically been found to be a good value. Values smaller than this may indicate an over-fit.

In line with \cite{robovj} the following parameters are set: $minHitRate = 0.999$. $maxFalseAlarmRate=0.5$. $numPos$ is chosen to be 90\% the positive samples, which is a conservative figure that provides a buffer of images to compensate for the positive cases falsely not detected anymore by the classifier. A common choice for $numNeg$ is twice $numPos$.

Grid search, stages, threshold. 
...
An abalation study is now performed in order to achevie a real time detector
