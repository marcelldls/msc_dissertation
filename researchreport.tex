...
More recent architectures have seen great success by encouraging models to learn from more challenging examples, rather than focusing on ``easy'' examples, by adding an additional factor to the cross-entropy loss. This new loss function is called the ``focal loss'' which aims to reduce the weighting of easy examples. \citep{focal}
...
In the basic YOLO architecture described by \cite{yolo}, the image is divided into an $S \times S$ grid  - which is achieved by down-sampling with a CNN backbone. Each grid cell predicts $B$ bounding boxes with associated objectness scores (probability of an object in the given region) and $C$ class probabilities. As per \cite{yolo} some disadvantages of this approach is that the detector is sensitive to these hyper-parameters and has difficulty detecting objects much smaller than the anchors as well as suffering from some localization error: \textit{``While it can quickly identify objects in images it struggles to precisely localize some objects, especially small ones''}. 

In order to achieve an additional performance boost, the YOLOv2 architecture by \cite{yolov2} introduces, amongst other tweaks, the usage of various predefined bounding boxes which are used as initial guesses in order to facilitate the model learning more diverse object sizes and aspect ratios. These are known as ``anchor'' boxes, with the technique being known as an anchor based approach. The box dimensions (AKA dimensional priors) are learnt by k-means clustering of the training data. 
