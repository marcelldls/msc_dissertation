...
In the YOLO architecture described by \cite{yolo}, the image is divided into an $S \times S$ grid. Each grid cell is used to predict $B$ bounding boxes with associated objectness scores (probability of an object in the given region) and $C$ class probabilities which is demonstrated in figure \ref{fig:yolodemo}. As per \cite{yolo} some disadvantages of this approach is that the detector is sensitive to these hyper-parameters and has difficulty detecting objects much smaller than the anchors as well as suffering localization error: \textit{``While it can quickly identify objects in images it struggles to precisely localize some objects, especially small ones''}. 

In order to achieve an additional performance boost, the YOLOv2 architecture by \cite{yolov2} introduces (amongst other incremental improvements) the usage of various predefined bounding boxes which are used as initial guesses in order to facilitate the model learning more diverse object sizes and aspect ratios. These are known as ``anchor'' boxes, with the technique being known as an anchor based approach. The box dimensions (AKA dimensional priors) are learnt by k-means clustering of the training data. Further incremental improvements have been made with YOLOv3 \citep{yolov3} and YOLOv4 \citep{yolov4}, with the most significant enhancements being a deeper more optimized backbone as well various pre and post-processing tricks to improve the efficiency of training. 
...
Although over estimates. We see for data set error is reduced.
