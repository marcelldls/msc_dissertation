...
Although a more sophisticated approach, in figure \ref{fig:ekferror} the EKF still suffers from some of the same simplistic assumptions as used for the first order Kalman filter. The EKF is not an optimal tracker and therefore is either detrimental or causes negligible improvement to the performance compared to no tracking being used.
...
Initially, the network predicts a Kalman gain close to 1. This is conisitent with the finidngs that the measurements are initally accuracye
As the ball travels further away. as the agent oerfrms the kick. The The kalman gain is reduced which means that the dynamics model is used. Therefore it is aoparent that the modelhas learnt an behavior which is a smarter initikaisation. It also shows that the model has learnt to rely on the measurmeents when the ball is near. 
The transition is sharp - which may indictate that at this point the model rcognised that a kick has occured
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusion}
