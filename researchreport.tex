...
The detection dataset is generated in the RoboCup ecosystem of tools. Approximately half the images are acquired using an 11 vs. 11 match. The remainder are collected in a training pitch environment.

\subsection{Software environment}
The detection dataset is generated using the RoboCup match server rcssserver3d-0.7.3 which is built on the simspark-0.3.2 simulator. In the simulator, the ``agentSyncMode'' variable is changed to true to enable synchronized execution. In the match server: ``enableRealTimeMode'' is set to false, ``addNoise'' is set to false and an ``ImagePerceptor'' is added to the Nao model to enable vision to be appended to the server messages sent to each agent.  

The agent message parser is extended to include the vision messages which are received in a base64 encoded format. Each image is stored as a .json file, since it is human readable, which includes the image metadata such as the spherical coordinates of the ball (vertical angle, horizontal angle, distance) and the image size.
...
Control of the agent is abstracted by using magmaFatProxy-1.2.2, which also needs to be extended to include the addition of vision messages.

\subsection{Preprocessing}
Image transform

Dataloaders are made for the VJ and coco datasets.
