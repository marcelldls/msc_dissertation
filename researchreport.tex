...
It is reported that components do not have equal importance for performance, which is relevant when real-time performance is desired. \cite{diagnosingtrack} conclude, \textit{``Contrary to popular belief, it turns out that the observation model (which is the focus of many papers on visual tracking) does not play the most important role in a tracker''}. Feature extraction is found to affect the performance most - which is a task that CNNs excel at. An object tracker which utilities the progress of CNN detection methods is a popular algorithm: ``Simple Online and Realtime tracking'' (SORT) \citep{sort}.

\subsection{SORT}

SORT is a framework which is based on the Kalman filter \citep{kalman} and Hungarian method \citep{hungarian} for visual object tracking. The Kalman filter is a recursive state estimator which uses a model to predict the state of a linear system, followed by a correction step using current observations to update the object’s state \citep{trackbook}. 

\subsubsection{Kalman filtering}

In this model, object motion can be described as a discrete linear time invariant (LTI) state-space process which defines the evolution of the state from time $t-1$ to time $t$ as:
\begin{equation} 
...
\end{equation}
Where $z_{t}$ is the object state at time $t$ of ($m \times 1$) shape, $H$ is the measurement matrix of ($m \times n$) shape which maps measurements to the object state. $\upsilon$ is the measurement noise vector which is normally distributed about a mean of zero with variance $R$ which accounts for measurement uncertainty.
...
\end{equation}
Systems with nonlinear state transition or measurement mappings can be linearized at each time step, using the Taylor series expansion, and then used in the standard equations to obtain the Extended Kalman Filter (EKF) \citep{trackbook}.
\begin{equation}
...
\end{equation}
The resulting matrices can each be described as the Jacobian respectively (first-order partial derivatives). $\mu$ refers to the state about which the linearization occurs. The EKF is non-optimal however remains powerful and is popularly applied.
