...
In order to classify each window, there are a significant number of Haar-like features possible at each possible detection. An efficient approach to classification, based on the adaBoost machine learning algorithm, is used as a ``feature selection'' process by training and selecting a subset of weak classifiers that can be combined into a strong classifier. The set of ``cascading classifiers'' are arranged as a degenerate decision tree, where each node is a stage of increasing complexity. False classifications cause an early exit from the cascade, therefore background regions are usually rejected quickly and more computation is spent on distinguishing complex regions. Multiple stages can be built, which are progressively more "difficult" to pass.

The speed of processing an image is therefore impacted by the complexity of the image (complex backgrounds can slow it down). Using less stages and features can result in faster compute times. Faster speeds can also be achieved by increasing the step sizes and scaling factor.
...
The training of the detector is done as described by \cite{vjdet}. Rather than trying to find an optimum number of stages, features per stage and threshold per stage -- which is considered to be a difficult problem -- a goal is chosen for the minimal desired hit rate for each stage of the classifier and the maximal desired false alarm rate for each stage of the classifier. The hit rate defines how much positive object training data can be misclassified. 

Each stage
in the cascade increases the false positive rate and decreases
the detection rate.

The false alarm rate sets the rate at which false positives occur and therefore dictates how many features are included in each stage. 0.5 would be equivalent to a random classifier. Lower values results in larger weak classifiers 
...
the minimum reduction in false positives and the maximum decrease in detection.


a simple approach is used to produce
an effective classifier which is highly efficient. Each stage
in the cascade reduces the false positive rate and decreases
the detection rate.  Each stage is trained by adding features until the
target detection and false positivesrates are met ( these rates
...
In line with \cite{robovj} the following parameters are set: $minHitRate = 0.999$. $maxFalseAlarmRate=0.5$. $numPos$ is chosen to be 90\% the positive samples, which is a conservative figure that provides a buffer of images to compensate for the positive cases falsely not detected anymore by the classifier.
