...
At close range, after agent has performed the kick, the Kalman gain remains consistent but is not static. In this manner the prediction benefits from accurate detections and retains responsiveness. As the ball travels further in distance within the region of reliable detection the rolling average Kalman gain drops only slightly. With a gain near 1, the neural Kalman filter tracks the trajectory similarly to the latch tracker (a reflection of the raw detections) rather than the ground truth. An explanation for a lack of learning here is likely that the loss resulting from such a case is very small compared to the loss from range limited detections. This argument may also explain why there seems to be no noise attenuation -- simply the error reduction is imperceptible.
...
The error of the trackers is plotted in figure \ref{fig:evalerror}:
...
Overall it can be seen that of all the trackers considered, neural Kalman filtering has achieved the lowest root mean square error against the test dataset. The majority of the performance arises from the long kciks
