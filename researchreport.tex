...
The scores are the highest of any of the considered models, without even performing a search for ideal hyper parameters. For this reason, a search is not performed beyond the architecture variations as the results are satisfactory and sufficient to prove the respective hypothesis. 

It is seen in general, that the lower resolution architectures perform inference at a higher rate, however, at a cost of inference quality. The $1.5\times$ models which have a deeper backbone perform inference slower, however, in this case they do not perform at a higher inference quality. This may be due to not searching for optimum parameters as models of greater complexity have a greater tendancy to overfit on a dataset. The $NanoDet-plus-m\_416$ instance is chosen as it provides the maximum performance at a speed that provides real-time inference.
...
Relatively, the VJ detector performs the worst of the detectors across the data. Despite the effort of searching for ideal parameters and tuning, it does not out perform any trained detector. Infact, a pre trained detector can provide better inference quality at a fraction of the effort and complexity 

For the easiest examples, NanoDet scores a near perfect result. 
...
\subsubsection{Obejct size}
saadsasd

\subsubsection{Sythnetic}
saadsasd

\subsubsection{Best Performer}
saadsasd
