...
\phantomsection
\begin{abstract}
Computer vision has the ability provide an abundance of environmental information to a robotic system, which makes perception and interaction with a dynamic world possible. Modern neural methods have significantly improved on traditional approaches to computer vision as measured by the quality of inference, especially in the popular object-detection problem space. However, many neural network models are focused on accuracy and are often relatively computationally heavy and large in size. In practice, robots have computational limitations due to spacial and power constraints which means that achieving practical real-time performance requires careful consideration of the accuracy versus latency trade-off. In this research proposal, the problem of real-time ball tracking in the context of simulated RoboCup football is considered. A tracking-by-detection framework is proposed, to take advantage of the progress of modern detection models, as well as the use of deep neural network assisted Kalman filtering for tracking, which strikes a balance between the ease of interpretation of the traditional model-based approach and the ability to learn complex dynamics using neural networks. 
...
\phantomsection
\begin{declaration}
I, Marcell Nagy, hereby uniquely declare the contents of this research proposal to be by my own efforts.
This proposal is prepared for the degree of Master of Science (Robotics) at the University of the Witwatersrand.
...
Thank you Prof. Richard Klein and Dr. Pravesh Ranchod for your constructive comments and feedback.
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=16cm]{images/Tracking_flowchart.jpeg}
...
\begin{itemize}
    \item Initialization -- An object to be tracked must be selected. This may be done manually or using an object detector.
    ...
    \item Observation model -- Classify if the candidate is the tracked object. Assigns detections to existing targets
    \item Model Updater -- Adaptation of the observation model.
\end{itemize}

It turns out that components do not have equal importance for performance, which is relevant when efficient computation for real-time performance is desired. \cite{diagnosingtrack} conclude, \textit{``Contrary to popular belief, it turns out that the observation model (which is the focus of many papers on visual tracking) does not play the most important role in a tracker''}. Rather, it is found that feature extraction affects the performance most - which is a task that CNNs excel at. An object tracker which capitalises on the modern achievements of CNN based detection methods is a popular algorithm: ``Simple Online and Realtime tracking'' (SORT) \citep{sort}.

\subsection{SORT}

SORT is a framework which is based on the Kalman filter \citep{kalman} and Hungarian method \citep{hungarian} for tracking. The Kalman filter is a recursive state estimator which uses a model to predict the state of a linear system, followed by a correction step using current observations to update the object’s state \citep{trackbook}. 

The prediction step is computed as:
\begin{gather*} 
X^{P}_{t} = A_{t}X_{t-1}  \\
\Sigma^{P}_{t} = A_{t}\Sigma_{t-1}A_{t}^{T} + \Sigma^{\eta}_{t}
\end{gather*}
Where $X^{P}_{t}$ and $\Sigma^{P}_{t}$ are the state and the covariance predictions at  time $t$. $\Sigma^{\eta}$ is the process error covariance. $A_{t}$ is the state transition matrix (from a system dynamics model) which describes how the state evolves between time $t$ and $t-1$. 

The correction step is computed as:
\begin{gather*} 
G_{t} = \Sigma^{P}_{t}D^{T}_{t}[D_{t}\Sigma^{P}_{t}D^{T}_{t} + \Sigma^{\xi}_{t} ]^{-1} \\
X_{t} = X^{P}_{t} + G_{t}[Y_{t}-D_{t}X^{P}_{t}] \\
\Sigma_{t} = \Sigma^{P}_{t} - G_{t}D_{t}\Sigma^{P}_{t}
\end{gather*}
$\Sigma^{\xi}_{t}$ is the measurement error covariance. $G_{t}$ is known as the Kalman gain, which here is optimally computed for mean-square error (for linear systems with gaussian noise) to apply a correction based on the relative uncertainty between the process and measurement. $Y_{t}$ is the current observation and $D_{t}$ is the measurement matrix which maps observations to the state. Systems with nonlinear state transition or measurement mappings can be linearized using the Taylor series expansion to obtain the Extended Kalman Filter (EKF) \citep{trackbook}.
...
Where $\epsilon_{i}$ is the error for the $i$th sample which is difference between the predicted and ground truth state. 
%RMSE is often used as a statistical design criterion for noisy systems as it prescribes a soft bounds based on probability (for true Gaussian noise it is by definition that 99.7\% of samples will occur within three standard deviations).
...
Some of the traditional object detectors that had relative success and some real-time application include the Viola Jones algorithm \citep{vjdet} as well as the HOG features and SVM classification based Dalal and Triggs (D\&T) detector \citep{hog}. \cite{whyDT} state, \textit{``Despite visual object detectors improving by leaps and bounds in the last decade, they still largely rely on the same underlying principles: linear Support Vector Machines (SVMs) trained on Histogram of Oriented Gradient (HOG) features''}.

Before the dramatic growth and change of focus towards modern computer vision techniques, traditional object detectors were reaching saturation. \citep{rcnn} write this regarding the matter: \textit{"Object detection performance, as measured on the canonical PASCAL VOC dataset, has plateaued in the last few years. The best-performing methods are complex ensemble systems that typically combine multiple low-level image features with high-level context"}.

\newpage
\subsection{Modern Object detection}

Deep learning approaches are considered to have become the state of the art since the year 2012 with AlexNet becoming the first CNN to lead the ImageNet Large Scale Visual Recognition Challenge (ILSVRC) \citep{alexnet}. Modern object detectors are able to achieve near perfect results on data-sets that traditional architectures fail to show even marginal comprehension of. This can be seen in the benchmarking of the You Only Look Once (YOLO) object detector (See figure \ref{fig:yoloplot}) \citep{yolo} versus the D\&T detector. Newer, more challenging benchmarks such as Microsoft COCO are developed to challenge the modern state-of-the-art. 
...
Modern object detectors utilize deep CNNs in order to learn feature extraction. CNNs are able to extract both low level and high level contextual features due to a hierarchical network structure. The networks can be trained by minimizing a loss function that combines the regression and classification performances. The first widely popularized object detector to make use of CNNs is R-CNN (Regions with CNN features) \citep{rcnn}, which uses CNNs for feature extraction but otherwise remains similar to a traditional detector. Modern detectors which utilize a region proposal stage and are known as two-stage detectors \citep{comprehensive}. 

%\begin{figure}[h!]
%\begin{center}
%\includegraphics[width=12cm]{images/timeline.jpg}
%\caption{Object detection timeline \citep{dlreview}}
%\label{fig:timeline}
%\end{center}
%\end{figure}

A breakthrough for object detection was achieved with the introduction of the one-stage detector YOLO by \cite{yolo} which achieved real-time speeds whilst remaining competitive with state-of-the-art detectors. The speed of single stage detectors is in general considered to be faster than that of two-stage detectors -- but at a cost of accuracy. In the YOLO architecture, various predefined ``anchor'' boxes are placed and evaluated. As per \cite{yolo} some disadvantages of this approach is that the detector is sensitive to these hyper-parameters and has difficulty detecting objects much smaller than the anchors as well as suffering from some localisation error: \textit{``While it can quickly identify objects in images it struggles to precisely localize some objects, especially small ones''}.

More recent advancements include anchor box free detectors -- which reformulate the problem as a keypoint detection. This frees the detector from the anchor box hyperparameters such as size and aspect ratio as well as allowing for simpler architecture which allow for faster computation. Although CornerNet \citep{cornernet} was proposed as the first alternative to the anchor-based approaches, more recent state-of-the-art models such as CenterNet \citep{centernet}, YOLOX \citep{yolox} and NanoDet \citep{nanodet} have been proposed which achieve real-time speeds and a higher accuracy due to a simplified keypoint estimation. In a keypoint based approach, a convolutional neural network is used to predict heatmaps, with the intensity peaks representing corners or object centers. The dimensions of the  bounding boxes are then predicted. \cite{comprehensive} report: \textit{``The keypoint-based methods generally outperform the anchor-based and two-stage methods in terms of accuracy and speed''} 

%\cite{comprehensive} prepare a comprehensive benchmarking of modern object detectors for real-time application, which can be consulted to compare architectures.
...
%
%In practice, a trade off exists between the real-time performance and accuracy achieved by a model when performing inference. Intuitively, when less data is taken into consideration such as by the reduction of image resolution, inference can be faster as it requires less computation, however, the accuracy may decrease as the detail and structure of the data is lost. Therefore the performance of a computer vision algorithm should not be evaluated in isolation of latency for real time application. In general, many computer vision architectures tend to focus on accuracy and therefore may use model ensembling and multicrop methods which are too slow for practical implementation \citep{speedacc}.
%
%Deep learning models in particular tend to be large in both memory and computational demands relative to the capacity of typical volume and power constrained embedded robotics computers. \textit{This creates a problem for realizing pervasive deep learning, which requires real-time inference, with low energy consumption and high accuracy, in resource-constrained environments} \citep{quantization}. Apart from selecting efficient neural network architectures, quantization offers possible means to reduce memory consumption and computational costs of a model which can result in improved real-time performance -- if the appropriate operations are both software and hardware supported.
%
...
%
%The real values $r$ are related to quantization levels in the following manner:
%\begin{equation}
%Q(r) = int(r/S) - Z
%\end{equation}
...
%\begin{figure}[h!]
%\begin{center}
%\includegraphics[width=10cm]{images/onnx.jpg}
...
\chapter{Work Plan}
\vspace{-0.1cm}
The following work schedule is proposed: (a full-time commitment is assumed where items include time for reviewing additional literature, troubleshooting and reporting)

\newcounter{taskcount}
\newcommand\taskcount{\addtocounter{taskcount}{1}\arabic{taskcount}}

\setdatenumber{2023}{5}{15}

\newcommand\specdays[1]
{%
...
{\bf \#}		&{\bf Description}									&{\bf Start}	&{\bf Days}		\\\hline
\taskcount		&Project kickoff						 			&June 23, 2022	&\hspace{8pt}- 	\\\hline      
				&{\bf Phase 1:}																		\\\hline         
				&Detection dataset																	\\\hline
\taskcount		&Generate detection dataset				 			&\datedate		&\specdays{7} 	\\
\taskcount		&Translate dataset to VJ format			 		&\datedate		&\specdays{3} 	\\
\taskcount		&Translate dataset to COCO format			 		&\datedate		&\specdays{3} 	\\\hline
				&Baseline detection																	\\\hline
\taskcount		&Train and evaluate multi-scale VJ				 	&\datedate		&\specdays{4} 	\\    
\taskcount		&Train and evaluate Yolov4 Tiny				 	&\datedate		&\specdays{4} 	\\\hline               
				&CNN Object detection																\\\hline
\taskcount		&Evaluate pretrained candidates					&\datedate		&\specdays{7}	\\                
\taskcount		&Fine-tune object detectors							&\datedate		&\specdays{7}	\\         
\taskcount		&Evaluate object detectors							&\datedate		&\specdays{7}	\\  
\taskcount		&Optimize final model								&\datedate		&\specdays{7}	\\\hline
				&{\bf Phase 2:}																		\\\hline       
				&Tracking dataset																	\\\hline		\taskcount		&Generate detection dataset				 			&\datedate		&\specdays{7} 	\\\hline
				&Baseline object tracking															\\\hline
\taskcount		&Implement baseline tracking			           	&\datedate		&\specdays{7}	\\\hline
				&Neural assisted object tracking													\\\hline
\taskcount		&Train object tracker			         			&\datedate		&\specdays{14}	\\
\taskcount		&Optimize tracker									&\datedate		&\specdays{4}	\\\hline
				&Reporting																			\\\hline
\taskcount		&Compare tracker performance						&\datedate		&\specdays{7}	\\
\taskcount		&Finalize report									&\datedate		&\specdays{14}	\\
\taskcount		&Submission											&\datedate		&\hspace{8pt}-	\\\hline											
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusion}
The aim of visual object tracking is to use a sequence of image frames to robustly estimate the motion state of a target object.
Visual object tracking offers a versatile approach that is capable of acquiring an abundance of environmental information that can be used by robotic agents to perceive and interact with a dynamic environment in real-time. In this research report, the context of ball tracking in a simulated RoboCup soccer environment is considered. It has been proposed to take advantage of the progress of modern neural approaches in computer vision that have shown significant improvements in performance over traditional methods. Results from the popular problem space of object detection can be used for tracking by utilizing a tracking-by-detection framework. Correspondence can then be achieved through the the use of Kalman filtering which is a popular model based commonly used for state-estimation in the robotics domain. It is proposed to apply a neural assisted formulation of Kalman filtering, which strikes a balance between the ease of interpretation of a model-based approach and the ability to learn complex dynamics using neural networks. 
